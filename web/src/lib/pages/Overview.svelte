...
<div class="space-y-12 animate-in fade-in duration-700 w-full pb-20">
    ...
    {#if dailySummary && dailySummary.length > 0}
        <div class="hero bg-base-100 rounded-3xl border border-base-300 shadow-sm overflow-hidden">
            <div class="hero-content text-center py-12 px-8 flex-col w-full">
                <div class="max-w-3xl">
                    <h2 class="text-[10px] uppercase tracking-[0.4em] font-black text-primary opacity-50 mb-8">Daily_Perspective</h2>
                    <div class="space-y-6">
                        {#each dailySummary as line}
                            {#if line.toLowerCase().includes("no ") || line.toLowerCase().includes("unavailable")}
                                <p class="text-[10px] font-mono uppercase tracking-[0.2em] opacity-20 py-4 border-y border-base-content/5">
                                    {line}
                                </p>
                            {:else}
                                <p class="text-3xl font-light leading-tight tracking-tight text-base-content italic quote">
                                    "{line}"
                                </p>
                            {/if}
                        {/each}
    ...
    <div class="alert bg-base-100 border border-base-300 shadow-sm py-8">
        ...
        <div>
            ...
            <div class="text-xs opacity-60">You listened to 12% more music than yesterday. Your average temperature was 1.2°C lower.</div>
        </div>
    </div>

    <div class="flex flex-col items-center gap-4 pt-10 border-t border-base-300 opacity-20 hover:opacity-100 transition-all duration-500">
        <button class="btn btn-outline btn-xs font-mono tracking-widest px-8 hover:btn-primary uppercase" onclick={onRefresh}>Sync_Kernel</button>
    </div>
</div>
