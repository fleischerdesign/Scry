...
<div class="space-y-8 animate-in fade-in duration-500 w-full max-w-4xl pb-20">
					...
					<div class="card-body p-6">
						<div class="flex items-center justify-between mb-4">
							<div class="w-12 h-12 rounded-2xl bg-base-200 flex items-center justify-center group-hover:scale-110 transition-transform shadow-inner">
								<Icon icon="lucide:puzzle" class="w-6 h-6 opacity-40 group-hover:text-primary group-hover:opacity-100 transition-all" />
							</div>
							<div class="flex flex-col items-end">
								<span class="badge badge-ghost font-mono text-[9px] opacity-40">{plugin.version}</span>
								{#if plugin.capabilities.includes('oauth')}
									<div class="mt-1 flex items-center gap-1 text-[8px] font-black text-primary uppercase tracking-widest">
						...
						<h3 class="font-bold text-lg tracking-tight mb-1 uppercase italic">{plugin.name}</h3>
						<p class="text-xs opacity-50 line-clamp-2 leading-relaxed mb-6 h-8">
						...
						<div class="flex items-center justify-between pt-4 border-t border-base-300/50">
							...
							<div class="flex items-center gap-1 text-primary opacity-0 group-hover:opacity-100 transition-opacity">
								<span class="text-[10px] font-black uppercase tracking-widest">Configure</span>
	...
	{:else}
		<div class="card bg-base-200 border-2 border-dashed border-base-300 p-20 text-center rounded-[3rem]">
			<Icon icon="lucide:alert-circle" class="w-12 h-12 mx-auto mb-4 opacity-20" />
			<p class="font-mono text-xs uppercase tracking-widest opacity-40">No extensions detected</p>
		</div>
	{/if}
</div>
