    ...
    {:else if plugin}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="md:col-span-2 space-y-6">
                <div class="card bg-base-100 border border-base-300 shadow-sm rounded-[2rem] overflow-hidden">
                    <div class="p-6 bg-base-200/50 border-b border-base-300 flex items-center justify-between">
                        <span class="text-[10px] font-black uppercase tracking-widest opacity-60">Plugin Configuration</span>
                        <div class="badge badge-outline badge-xs opacity-40 font-mono tracking-tighter">ID: {plugin.id}</div>
                    </div>
                    <div class="p-8 space-y-6">
                        {#if plugin.config_schema}
                            ...
                            {/if}
                        {:else}
                            <div class="py-10 text-center opacity-30 italic text-sm">
                ...
                {#if plugin.capabilities.includes('oauth')}
                    <div class="card bg-base-100 border border-base-300 shadow-sm rounded-[2rem] overflow-hidden">
                        <div class="p-6 bg-base-200/50 border-b border-base-300 flex items-center gap-2">
                            <Icon icon="lucide:key-round" class="w-4 h-4 opacity-40" />
                            <span class="text-[10px] font-black uppercase tracking-widest opacity-60">Identity & Authentication</span>
                        </div>
                        <div class="p-8 flex flex-col md:flex-row items-center justify-between gap-6">
                            <div class="space-y-1">
                                <h4 class="font-bold text-sm">OAuth Connection</h4>
                                <p class="text-xs opacity-50">Authorize Scry to access your {plugin.name} account data.</p>
                            </div>
                            <button 
            ...
            <div class="space-y-6">
                <div class="card bg-base-100 border border-base-300 shadow-sm rounded-[2rem] overflow-hidden">
                    <div class="p-6 bg-base-200/50 border-b border-base-300 flex items-center gap-2">
                        <Icon icon="lucide:info" class="w-4 h-4 opacity-40" />
                        <span class="text-[10px] font-black uppercase tracking-widest opacity-60">Extension Info</span>
                    </div>
                    <div class="p-6 space-y-4">
                        <div class="flex justify-between items-center py-2 border-b border-base-300/50">
                            <span class="text-[10px] font-bold opacity-40 uppercase">Version</span>
                            <span class="badge badge-ghost font-mono text-[10px]">{plugin.version}</span>
                        </div>
                        <div class="flex justify-between items-center py-2 border-b border-base-300/50">
                            <span class="text-[10px] font-bold opacity-40 uppercase">Status</span>
                        ...
                        <div class="flex justify-between items-center py-2">
                            <span class="text-[10px] font-bold opacity-40 uppercase">Polling</span>
                ...
                <div class="card bg-base-100 border border-base-300 shadow-sm rounded-[2rem] overflow-hidden">
                    <div class="p-6 bg-base-200/50 border-b border-base-300 flex items-center gap-2">
                        <Icon icon="lucide:layers" class="w-4 h-4 opacity-40" />
                        <span class="text-[10px] font-black uppercase tracking-widest opacity-60">Capabilities</span>
                    </div>
                    <div class="p-6 flex flex-wrap gap-2">
                        {#each plugin.capabilities as cap}
                            <div class="badge badge-ghost border-base-300 font-mono text-[9px] uppercase">{cap}</div>
                        {/each}
                    </div>
                </div>
            </div>
