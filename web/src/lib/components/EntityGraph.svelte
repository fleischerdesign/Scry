...
>
        ...
        <g transform="translate({translateX}, {translateY}) scale({scale})">
            ...
            {#each nodes.slice(1) as node, i}
                ...
                <g transform="translate({edge.x}, {edge.y})">
                    ...
                    <text text-anchor="middle" dy="4" class="fill-base-content/50 font-black text-[8px] uppercase tracking-tighter">{edge.label}</text>
                </g>
            {/each}
            ...
            {#each nodes.slice(1) as node}
                <g class="cursor-pointer group" onclick={(e) => { e.stopPropagation(); router.navigate(`/entity/${node.ns}/${node.typ}/${node.id}`); }}>
                    ...
                    <g class="opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                        ...
                        <text x={node.x} y={node.y + 41} text-anchor="middle" class="fill-white font-bold text-[9px] uppercase tracking-widest">{node.title}</text>
                    </g>
                </g>
            {/each}
            ...
            <g class="filter drop-shadow-2xl">
                ...
                {:else}
                    <text x={centerX} y={centerY} dy="6" text-anchor="middle" class="fill-primary-content font-black text-xl italic tracking-tighter">{centralTitle.charAt(0).toUpperCase()}</text>
                {/if}
    ...
    <!-- Legend Hint -->
    <div class="absolute bottom-8 left-10 flex flex-col gap-2 opacity-40 pointer-events-none">
        <div class="flex items-center gap-2">
            <div class="w-2.5 h-2.5 rounded-full bg-primary shadow-sm shadow-primary/50"></div>
            <span class="text-[9px] font-black uppercase tracking-widest text-base-content">Active Focus</span>
        </div>
        <div class="flex items-center gap-2">
            <div class="w-2.5 h-2.5 rounded-full bg-base-300 border border-base-content/10"></div>
            <span class="text-[9px] font-black uppercase tracking-widest text-base-content">Connected Node</span>
        </div>
    </div>
