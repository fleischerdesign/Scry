...
<aside class="w-32 hidden lg:flex flex-col sticky top-24 h-fit pr-4 border-r border-base-300 mr-8">
    <div class="flex flex-col gap-1">
        <div class="flex items-center gap-2 mb-4 px-2">
            <Icon icon="lucide:calendar" class="w-3 h-3 opacity-30" />
            <span class="text-[10px] font-black tracking-[0.2em] opacity-30 uppercase">{activeYear}</span>
        </div>
        ...
        {#each allMonths as month}
            ...
            >
                <span class="text-[10px] font-mono font-bold tracking-widest {month.hasData ? 'text-base-content opacity-60 group-hover:text-primary group-hover:opacity-100' : ''}">
                    {month.label}
                </span>
                
                {#if month.hasData}
                    <div class="w-1 h-1 rounded-full bg-primary opacity-40 group-hover:scale-150 transition-transform"></div>
                {/if}
            </button>
        {/each}
    </div>
