...
<li>
 ...
 <div class="timeline-start font-mono text-xs opacity-70">{time}</div>
   ...
   <div class="flex flex-col gap-1 flex-1 overflow-hidden">
    <div class="flex justify-between items-start">
     <div class="flex flex-col flex-1">
      <span class="text-xs font-bold tracking-tight truncate">
     ...
     <span class="badge badge-ghost badge-xs opacity-60 lowercase shrink-0 ml-2">
    ...
    <div class="flex flex-wrap gap-1 mt-1">
     ...
     {#each contextMetrics as metric}
      <div 
       ...
       class="badge badge-accent/10 text-accent border-accent/20 badge-xs gap-1 font-bold {metric.source_id ? 'hover:bg-accent/20 cursor-pointer transition-colors' : ''}"
      >
       <span class="opacity-70 text-xs tracking-wide">{semanticService.getLabel(metric.key)}:</span>
     ...
     {#if item.context && item.context.length > 0}
      {#each item.context as hint}
       <div class="badge badge-primary/10 text-primary border-primary/20 badge-xs gap-1 font-bold tracking-tighter">
