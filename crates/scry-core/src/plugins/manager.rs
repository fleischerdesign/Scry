...
impl PluginManager {
    ...
    fn setup_linker(&self) -> Result<Linker<MyCtx>> {
        ...
        crate::plugins::Plugin::add_to_linker::<MyCtx, wasmtime::component::HasSelf<MyCtx>>(
            &mut linker,
            |state| state,
        )?;
        Ok(linker)
        ...
        plugins
            ...
            .filter(|(_, p)| {
                p.manifest
                    .subscriptions
                    .iter()
                    .any(|s| Self::matches_subscription(s, category))
            })
        ...
        Fut: std::future::Future<Output = Result<(T, Store<MyCtx>)>>,
    {
        ...
        match timeout(timeout_duration, f(instance_wrapper, store)).await {
            ...
            Err(_) => {
                tracing::error!(
                    "Plugin {} execution timed out after {}s",
                    name,
    ...
    pub async fn reload_plugins(&self) -> Result<()> {
        ...
        let futures = entries
            .into_iter()
            .map(|path| self.load_plugin_single(path));
    ...
    async fn load_plugin_single(
        &self,
        path: std::path::PathBuf,
    ) -> Result<(String, PluginInstance)> {
        let name = path
            .file_stem()
            .and_then(|s| s.to_str())
            .map(|s| s.to_string())
    ...
    pub async fn run_ingest_pipeline(
        ...
        for name in plugin_names {
            let res = self
                .with_instance(&name, user_id, |instance, mut store| async move {
                    let wit_event = crate::plugins::scry::plugin::types::Event::from(&event);
                    let processed = instance
                        .call_on_ingest(&mut store, &wit_event)
                        .await?
                        .map_err(|e: String| anyhow::anyhow!(e))?;
                    let mapped = ScryEvent::try_from(processed)
                        .map_err(|e: ConversionError| anyhow::anyhow!(e))?;
                    Ok((mapped, store))
                })
                .await?;
            event = res;
    ...
    pub async fn list_plugin_reports(
        &self,
        user_id: i64,
    ) -> Result<Vec<(String, Vec<ReportMetadata>)>> {
        let mut all = Vec::new();
    ...
    pub async fn run_plugin_report(
        ...
        self.with_instance(plugin_name, user_id, |instance, mut store| async move {
            let res = instance
                .call_run_report(&mut store, &report_id)
                .await?
                .map_err(|e: String| anyhow::anyhow!(e))?;
    ...
    pub async fn get_plugin_summary(
        &self,
        user_id: i64,
        plugin_name: &str,
        start: String,
    ...
    pub async fn notify_entity_discovered(
        &self,
        user_id: i64,
        namespace: String,
        typ: String,
        ...
        for name in plugin_names {
            ...
            self.with_instance(&name, user_id, |instance, mut store| async move {
                instance
                    .call_on_entity_discovered(&mut store, &ns, &t, &i)
                    .await?;
                Ok(((), store))
            })
            .await?;
