...
use crate::plugins::scry::plugin::host::{
    Host, HttpRequestData, HttpResponse, QueryParam, Relationship,
};
...
impl Host for MyCtx {
                ...
                let val = match row.try_get_raw(col.ordinal()) {
                    Ok(raw) if !raw.is_null() => {
                        if let Ok(v) = row.try_get::<i64, _>(col.ordinal()) {
                            ...
                            tracing::warn!(
                                "Failed to map column '{}' with type {:?}",
                                col.name(),
                                col.type_info()
                            );
    ...
    async fn log(&mut self, level: String, message: String) -> Result<()> {
        match level.to_lowercase().as_str() {
            "error" => {
                tracing::error!(plugin_id = %self.plugin_name, user_id = %self.user_id, "{}", message)
            }
            "warn" => {
                tracing::warn!(plugin_id = %self.plugin_name, user_id = %self.user_id, "{}", message)
            }
            _ => {
                tracing::info!(plugin_id = %self.plugin_name, user_id = %self.user_id, "{}", message)
            }
        }
        Ok(())
    }

    async fn set_entity_trait(
        &mut self,
        namespace: String,
        typ: String,
        id: String,
        ...
        self.state_repo()
            .set_trait(&namespace, &typ, &id, &trait_id, &value_json)
            .await
            .map_err(|e| anyhow::anyhow!(e))
    }

    async fn get_entity_trait(
        &mut self,
        namespace: String,
        typ: String,
        id: String,
        ...
        self.state_repo()
            .get_trait(&namespace, &typ, &id, &trait_id)
            .await
            .map_err(|e| anyhow::anyhow!(e))
    }
    ...
    async fn get_relationships(
        ...
        self.state_repo()
            .get_relationships(&namespace, &typ, &id, &direction)
            .await
            .map_err(|e| anyhow::anyhow!(e))
    }
