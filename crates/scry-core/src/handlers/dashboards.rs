...
pub async fn create_dashboard(
    ...
    state
        .dashboard_service
        .create_dashboard(auth.user_id, name)
        .await?;
    Ok(StatusCode::OK)
}
...
pub async fn delete_widget(
    State(state): State<Arc<AppState>>,
    Path((_id, widget_id)): Path<(String, String)>,
    Extension(auth): Extension<AuthContext>,
) -> Result<impl IntoResponse> {
    state
        .dashboard_service
        .delete_widget(auth.user_id, &widget_id)
        .await?;
    Ok(StatusCode::OK)
}
...
pub async fn add_widget(
    ...
    let w_type = req["type"]
        .as_str()
        .ok_or_else(|| Error::BadRequest("Missing type".to_string()))?;
    let title = req["title"].as_str();

    state
        .dashboard_service
        .add_widget(auth.user_id, &id, w_type, title, req["config"].clone())
        .await?;
