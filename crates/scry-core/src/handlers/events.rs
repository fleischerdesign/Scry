...
pub async fn get_daily_summary(
    ...
    let summary = state
        .event_service
        .generate_daily_summary(auth.user_id, date)
        .await?;
    Ok(Json(summary))
}
...
pub async fn get_event_by_id(
    ...
    let ev: Option<Event> = state
        .event_service
        .get_event_by_id(auth.user_id, &id)
        .await?;
...
pub async fn ingest_event(
    ...
    let event = state
        .event_service
        .ingest_event(auth.user_id, event)
        .await?;
    Ok(Json(event))
}
...
pub async fn get_events_by_entity(
    ...
    let events: Vec<Event> = state
        .event_service
        .get_events_by_entity(auth.user_id, &namespace, &typ, &id)
        .await?;
