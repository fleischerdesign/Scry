...
pub async fn trigger_discovery(
    ...
    let count = state
        .analytics_service
        .run_correlation_discovery(auth.user_id)
        .await?;
    Ok(Json(
        json!({ "status": "success", "new_discoveries": count }),
    ))
}
...
pub async fn get_semantic_stats(
    State(state): State<Arc<AppState>>,
    Query(params): Query<CorrelateParams>,
    Extension(auth): Extension<AuthContext>,
) -> Result<Json<SemanticStats>> {
    let bs = params
        .base_semantic
        .as_ref()
        .ok_or_else(|| Error::BadRequest("base_semantic required".to_string()))?;
    let js = params
        .join_semantic
        .as_ref()
        .ok_or_else(|| Error::BadRequest("join_semantic required".to_string()))?;
    let stats = state
        .event_service
        .calculate_semantic_stats(auth.user_id, bs, js, params.limit.unwrap_or(100))
        .await?;
    Ok(Json(stats))
}
...
pub async fn correlate_events(
    ...
    Query(params): Query<CorrelateParams>,
    Extension(auth): Extension<AuthContext>,
) -> Result<Json<Vec<CorrelationResult>>> {
    let limit = params.limit.unwrap_or(50);
    let results: Vec<serde_json::Value> =
        if let (Some(bs), Some(js)) = (&params.base_semantic, &params.join_semantic) {
            state
                .event_service
                .correlate_semantic(auth.user_id, bs, js, limit)
                .await
        } else if let (Some(bc), Some(jc)) = (&params.base_category, &params.join_category) {
            state
                .event_service
                .correlate_nearest(auth.user_id, bc, jc, limit)
                .await
        } else {
            return Err(Error::BadRequest("Invalid params".to_string()));
        }?;
    let api_results = results
        .into_iter()
        .map(|v| CorrelationResult {
...
pub async fn search_events(
    ...
    let raw_results = state
        .analytics_service
        .search(auth.user_id, q, params.limit.unwrap_or(20))
        .await?;
        ...
        if typ == "event" {
            if let Ok(Some(ev)) = state.event_service.get_event_by_id(auth.user_id, id).await {
                enriched["display_title"] =
                    json!(ev.display_title.unwrap_or_else(|| ev.category.clone()));
            ...
            if parts.len() >= 5 {
                ...
                let e_type = parts[3];
                let e_id = parts[4];
                let entity_repo = crate::repository::EntityRepository::new(
                    state.event_service.db(),
                    auth.user_id,
                );
                let (title, _subtitle, image, _icon) =
                    entity_repo.get_display_info(ns, e_type, e_id).await;
