...
impl PluginService {
    ...
    pub async fn poll_plugin_manually(&self, user_id: i64, plugin_id: &str) -> Result<usize> {
        self.event_service
            .poll_and_save_plugin(user_id, plugin_id)
            .await
    }
    ...
    pub async fn run_plugin_report(
        ...
        plugin_id: &str,
        report_id: String,
    ) -> Result<ApiReportData> {
        let data = self
            .plugin_manager
            .run_plugin_report(user_id, plugin_id, report_id)
            .await?;
        Ok(data.into())
    }
    ...
    async fn get_config_schema(&self, plugin_id: &str) -> Option<serde_json::Value> {
        ...
        manifests
            .get(plugin_id)
            .and_then(|m| m.config_schema.as_ref())
            .and_then(|s| serde_json::from_str(s).ok())
    }

    fn extract_secret_keys(
        &self,
        ...
        let core_manifest = scry_plugin_sdk::Manifest {
            ...
            suggested_widgets: vec![
                ...
                scry_plugin_sdk::WidgetDefinition {
                    ...
                    config_json: json!({ "semantic_type": "system.entities", "days": 7 })
                        .to_string(),
                },
            ],
            oauth_config: None,
    ...
    pub async fn get_oauth_config(&self, plugin_id: &str) -> Result<scry_plugin_sdk::OAuthConfig> {
        ...
        let manifest = manifests
            .get(plugin_id)
            .ok_or_else(|| crate::error::Error::NotFound("Plugin not found".into()))?;

        manifest
            .oauth_config
            .clone()
            .ok_or_else(|| crate::error::Error::NotFound("OAuth not configured".into()))
    }
