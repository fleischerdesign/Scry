...
impl EventService {
    ...
    pub async fn enrich_event_context(&self, user_id: i64, event: &mut Event) -> Result<()> {
        ...
        for entity in &event.entities {
            ...
            if event.display_image.is_none() {
                let (_, _, image, _) = entity_repo.get_display_info(&entity.namespace, &entity.typ, &entity.id).await;
    ...
    pub async fn get_semantic_top(&self, user_id: i64, semantic_type: &str, limit: u32, days: Option<u32>) -> Result<Vec<Value>> {
        ...
        for item in raw_top {
            ...
            if parts.len() == 2 {
                let (title, _subtitle, image, _icon) = entity_repo.get_display_info(parts[0], parts[1], id).await;
