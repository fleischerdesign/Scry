...
use crate::repository::{AnalyticsRepository, ConfigRepository, EntityRepository, EventRepository};
...
impl EventService {
    ...
    pub async fn enrich_event_context(&self, user_id: i64, event: &mut Event) -> Result<()> {
        ...
        for m in manifests.values() {
            for export in &m.exports {
                if event.category != export.category {
                    context_categories
                        .insert(export.semantic_type.clone(), export.category.clone());
                }
            }
        }
            ...
            if let Some(other_event) = event_repo.get_last_event(cat, &ts).await? {
                ...
                if category == SemanticCategory::Metric {
                    if let Some(val) = other_event.display_value {
                        ...
                        metric_info
                            .insert("source_id".to_string(), json!(other_event.id.to_string()));
        ...
        for entity in &event.entities {
            ...
            if event.display_image.is_none() {
                let (_, _, image, _) = entity_repo
                    .get_display_info(&entity.namespace, &entity.typ, &entity.id)
                    .await;
                event.display_image = image;
            }

            if let Ok(traits) = entity_repo
                .get_traits(&entity.namespace, &entity.typ, &entity.id)
                .await
                && !traits.is_empty()
            {
                ...
                info.insert(
                    format!("traits:{}", entity.path),
                    Value::Object(entity_info),
                );
            }
        }
    ...
    pub async fn ingest_event(&self, user_id: i64, event: Event) -> Result<Event> {
        ...
        let mut processed_event: Event = self
            .plugin_manager
            .run_ingest_pipeline(user_id, event)
            .await
            .map_err(Error::Plugin)?;

        // --- Dynamic Context Resolution ---
        let hints = processed_event.context.clone();
        let processor_id = processed_event
            .metadata
            .as_ref()
            ...
            .unwrap_or_else(|| {
                processed_event
                    ...
                    .split('+')
                    .next()
                    .unwrap_or(&processed_event.source)
            });
        ...
        for hint in hints {
            let entity_ref = if hint.starts_with("alias:") {
                ...
                if let Some(target_uri) = config_repo.get(processor_id, &alias_key).await? {
                    ...
                    if let Some(target) = default_target {
                        let _ = config_repo
                            .set_if_not_exists(processor_id, &alias_key, target, false)
                            .await;
                        Some(scry_proto::EntityRef {
                            path: hint.clone(),
            ...
            if let Some(r) = entity_ref
                && !processed_event
                    .entities
                    .iter()
                    .any(|e| e.namespace == r.namespace && e.typ == r.typ && e.id == r.id)
            {
        ...
        let _ = self
            .enrich_event_context(user_id, &mut processed_event)
            .await;

        let mut meta = processed_event
            .metadata
        ...
        for ent in &processed_event.entities {
            entity_repo
                .ensure_entity(&ent.namespace, &ent.typ, &ent.id)
                .await?;
    ...
    pub async fn correlate_semantic(
        &self,
        user_id: i64,
        base_semantic: &str,
        join_semantic: &str,
        ...
        let manifests: HashMap<String, scry_plugin_sdk::Manifest> =
            self.plugin_manager.get_plugin_manifests().await;
        let mut base_cat: Option<String> = None;
        let mut join_cat: Option<String> = None;

        for m in manifests.values() {
            for export in &m.exports {
                if export.semantic_type == base_semantic {
                    base_cat = Some(export.category.clone());
                }
                if export.semantic_type == join_semantic {
                    join_cat = Some(export.category.clone());
                }
            }
        }
        ...
        match (base_cat, join_cat) {
            ...
            _ => Err(Error::NotFound(format!(
                "Semantic types {} or {} not found",
                base_semantic, join_semantic
            ))),
        }
    }
    ...
    pub async fn search_semantic(
        ...
        for cat in target_categories {
            all_events.extend(
                self.list_events(user_id, Some(cat), limit + offset, 0)
                    .await?,
            );
        }
    ...
    pub async fn get_enriched_timeline(
        &self,
        user_id: i64,
        base_category: Option<String>,
        limit: u32,
        offset: u32,
    ) -> Result<Vec<Event>> {
        let mut base_events = self
            .list_events(user_id, base_category, limit, offset)
            .await?;
    ...
    pub async fn correlate_nearest(
        ...
        let repo = AnalyticsRepository::new(&self.db, user_id);
        repo.correlate_nearest(base_category, join_category, limit)
            .await
    }
    ...
    pub async fn resolve_semantic_info(&self, semantic_type: &str) -> Result<(String, String)> {
        ...
        Err(Error::NotFound(format!(
            "Semantic type {} not found in catalog",
            semantic_type
        )))
    }
    ...
    pub async fn get_semantic_top(
        ...
        for item in raw_top {
            ...
            if parts.len() == 2 {
                let (title, _subtitle, image, _icon) =
                    entity_repo.get_display_info(parts[0], parts[1], id).await;
                enriched["display_title"] = json!(title);
                if let Some(img) = image {
                    enriched["display_image"] = json!(img);
                }
            }
    ...
    pub async fn get_semantic_series(
        ...
        let (category, path) = self.resolve_semantic_info(semantic_type).await?;
        let repo = AnalyticsRepository::new(&self.db, user_id);
        repo.get_semantic_series(&category, &path, days, interval)
            .await
    }

    pub async fn calculate_semantic_stats(
        &self,
        user_id: i64,
        base_semantic: &str,
        join_semantic: &str,
        ...
        let correlations = self
            .correlate_semantic(user_id, base_semantic, join_semantic, limit)
            .await?;
        ...
        Ok(SemanticStats {
            base_type: base_semantic.to_string(),
            join_type: join_semantic.to_string(),
            sample_size,
            correlations: serde_json::to_value(distribution).map_err(|_e| Error::Internal)?,
        })
    }
    ...
    pub async fn generate_daily_summary(&self, user_id: i64, date: &str) -> Result<Vec<String>> {
        ...
        for plugin_name in manifests.keys() {
            if let Ok(p_summary) = self
                .plugin_manager
                .get_plugin_summary(user_id, plugin_name, start.clone(), end.clone())
                .await
                && !p_summary.is_empty()
            {
                full_summary.push(p_summary);
            }
        }
        Ok(full_summary)
    }
...
mod tests {
    ...
    async fn test_ingest_and_list_events() {
        ...
        // List
        let events = service
            .list_events(1, Some("test.event".to_string()), 10, 0)
            .await
            .unwrap();
    ...
    async fn test_multi_tenancy_isolation() {
        ...
        let event_alice = Event::new(
            "private".to_string(),
            "alice-phone".to_string(),
            json!({"secret": "alice_data"}),
        );
