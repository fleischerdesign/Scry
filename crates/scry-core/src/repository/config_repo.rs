...
impl<'a> ConfigRepository<'a> {
    pub fn new(pool: &'a SqlitePool, user_id: i64, secret_service: &'a SecretService) -> Self {
        Self { pool, user_id, secret_service }
    }
    ...
    pub async fn set(&self, plugin_id: &str, key: &str, value: &str, is_secret: bool) -> Result<()> {
    ...
    pub async fn set_if_not_exists(&self, plugin_id: &str, key: &str, value: &str, is_secret: bool) -> Result<()> {
        let value_to_store = if is_secret {
            self.secret_service.encrypt(self.user_id, value)?
        } else {
            value.to_string()
