...
impl<'a> EntityRepository<'a> {
    ...
    pub async fn set_trait_if_not_exists(
        &self,
        namespace: &str,
        entity_type: &str,
        entity_id: &str,
    ...
    pub async fn get_namespaces(&self) -> Result<Vec<String>> {
        let names = sqlx::query_scalar::<_, String>(
            "SELECT DISTINCT namespace FROM entities WHERE user_id = ?",
        )
        .bind(self.user_id)
        .fetch_all(self.pool)
        .await?;
    ...
    pub async fn get_types_by_namespace(&self, namespace: &str) -> Result<Vec<String>> {
        let types = sqlx::query_scalar::<_, String>(
            "SELECT DISTINCT typ FROM entities WHERE user_id = ? AND namespace = ?",
        )
        .bind(self.user_id)
        .bind(namespace)
        .fetch_all(self.pool)
    ...
    pub async fn get_display_info(
        ...
        let title = self
            .get_trait(namespace, typ, id, scry_plugin_sdk::schema::traits::NAME)
            .await
            .ok()
            .flatten()
        ...
        let subtitle = self
            .get_trait(
                ...
                typ,
                id,
                scry_plugin_sdk::schema::traits::SUBTITLE,
            )
            .await
            .ok()
            .flatten();
        ...
        let mut image = self
            .get_trait(namespace, typ, id, scry_plugin_sdk::schema::traits::PHOTO)
            .await
            .ok()
            .flatten();
        if image.is_none() {
            image = self
                .get_trait(namespace, typ, id, scry_plugin_sdk::schema::traits::AVATAR)
                .await
                .ok()
                .flatten();
        }
        ...
        let icon = self
            .get_trait(namespace, typ, id, scry_plugin_sdk::schema::traits::ICON)
            .await
            .ok()
            .flatten();
